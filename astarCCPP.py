...
def frontier_size_heuristic(covergrid, x, y):
    frontier_size = 8
    for i in range(-1, 2):
        for j in range(-1, 2):
            if i == 0 and j == 0:
            ...
            new_x = x + i*GRID_SIZE
            new_y = y + j*GRID_SIZE
            if new_x >= 0 and new_x < covergrid.shape[0] and new_y >= 0 and new_y < covergrid.shape[1] and grid[int(new_x / GRID_SIZE)][int(new_y / GRID_SIZE)] == 0:
                if covergrid[i,j] == 0:
                    frontier_size -= 1
    return frontier_size

def update_covergrid(covergrid, closed_list):
    for node in closed_list:
        covergrid[int(node.x / GRID_SIZE)][int(node.y / GRID_SIZE)] = 1



def astar(start, grid, covergrid):
    ...
    start.g = 0
    start.f = frontier_size_heuristic(covergrid, start.x, start.y)
    ...
    while len(open_list) > 0 :
        ...
        while len(temp) > 0:
            ...
            heapq.heappush(temp2, k)
        while len(open_list) > 0 :
            t = heapq.heappop(open_list)
            heapq.heappush(temp, t)
        open_list.clear()
        covergrid[int(current.x / GRID_SIZE)][int(current.y / GRID_SIZE)] = 1
        print("covercount", np.count_nonzero(covergrid==1))
        #update_covergrid(covergrid, closed_list)
        # Check if goal is reached
        if  not any(0 in x for x in covergrid):
            path = []
            print("hello, world!")
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                neighbor_f = neighbor_g + frontier_size_heuristic(covergrid, neighbor_x, neighbor_y)
        ...
        if len(open_list) == 0:
            if len(temp) == 0:
                print("fuck")
                z = heapq.heappop(temp2)
                heapq.heappush(open_list, z)
            else:
                z = heapq.heappop(temp)
                heapq.heappush(open_list, z)
                print("shit")
...
covergrid = np.zeros((int(WINDOW_WIDTH/GRID_SIZE), int(WINDOW_HEIGHT/GRID_SIZE)), dtype=int)
set_obstacle(grid)
set_obstacle(covergrid)

start = Node(20, 20)


path = astar(start, grid, covergrid)
